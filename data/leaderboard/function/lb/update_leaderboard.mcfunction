...
$scoreboard players set lb_max temp_max $(max_players)
execute if entity @s[nbt={data:{always_show_closest_player:1}}] as @s at @s if score lb_temp temp_rank > lb_max temp_max if entity @p[distance=..10] run function leaderboard:lb/add_close_player_line with storage leaderboard:line
...
execute if entity @s[nbt={data:{always_show_closest_player:1}}] as @s at @s unless score lb_temp temp_rank > lb_max temp_max run function leaderboard:lb/remove_closest_line