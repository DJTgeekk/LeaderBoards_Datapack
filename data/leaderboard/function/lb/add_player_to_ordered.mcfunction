...
$execute if entity @s[nbt={data:{no_zero:1}}] unless score lb_max temp_score_display matches 0 run data modify storage leaderboard:temp_namelist names_ordered append from storage leaderboard:temp_namelist names_unordered[$(max_id)]
