...
$execute at @e[limit=1,sort=nearest,type=minecraft:text_display,tag=top,tag=leaderboard,tag=display_$(score)] run function leaderboard:commands/edit_leaderboard {max_players:$(max_players),score:$(score),reverse_order:$(reverse_order),time_mode:$(time_mode),no_zero:$(no_zero),display_name:"$(display_name)",billboard:$(billboard)}
