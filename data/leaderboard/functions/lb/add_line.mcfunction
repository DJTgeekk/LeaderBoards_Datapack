...
$execute at @s run data modify entity @e[distance=$(sep_min)..$(sep_max),type=text_display,tag=display_$(score),tag=!top,tag=$(lines),limit=1] Rotation set from entity @e[limit=1,sort=nearest,tag=top,tag=display_$(score)] Rotation
